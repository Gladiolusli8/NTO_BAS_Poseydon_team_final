...
CAM_ID = 0  # ID камеры (при подключении по USB)
...
if __name__ == "__main__":
    while cv2.waitKey(1000 // 60) != ord("q"):
        ...
        img = cv2.undistort(frame, cameraMatrix, distCoeffs=distCoeffs, newCameraMatrix=cameraMatrix)
        ...
        if ids is not None:
            ...
            for marker in range(len(ids)):
                ...
                img = cv2.drawFrameAxes(img, cameraMatrix, distCoeffs, rvec, tvec, 0.1, 5)
