...
def estimatePoseSingleMarkers(marker_points: cv2.typing.MatLike,
    ...
    if useEPNP:
        ...
        return cv2.solvePnP(marker_world_points, marker_points, cameraMatrix, distCoeffs,
...
def getFixedZWPosAll(src: [tuple, list, npt.ArrayLike], Zw: float, cameraMatrix: cv2.typing.MatLike,
                     wTc: npt.ArrayLike) -> (npt.ArrayLike, npt.ArrayLike):
    """
...
def getFixedZWPos(src: [tuple, list, npt.ArrayLike], Zw: float, cameraMatrix: cv2.typing.MatLike,
                  wTc: npt.ArrayLike) -> npt.ArrayLike:
    """
...
def positionMarker(
    ...
    marker_points = np.array(
        [[-marker_size / 2, marker_size / 2, 0], [marker_size / 2, marker_size / 2, 0],
         [marker_size / 2, -marker_size / 2, 0], [-marker_size / 2, -marker_size / 2, 0]],
        dtype=np.float32)  # Определение объектной модели ArUco маркера
    ...
    ay = np.arctan2(-1 * rvec[2][0], np.sqrt(
        (rot_mat[2][1]) ** 2 + (rot_mat[2][2]) ** 2))  # Нахождение угла Эйлера из матрицы поворота относительно OY
    az = np.arctan2(rot_mat[1][0], rot_mat[0][0])  # Нахождение угла Эйлера из матрицы поворота относительно OZ
    return np.array([[x, y, z], [ax, ay, az]], dtype=np.float32), np.array([rvec, tvec],
                                                                           dtype=np.float32)  # Возврат функции ([Координаты, Углы], [rvec, tvec])